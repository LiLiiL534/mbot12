...
class DcinsideBot:
    ...
    async def write_article(self, trending_topics, memory_data=None):
        ...
        - 페르소나를 완벽히 지키면서, 원신 유저들이 화낼만한 제목이나 클릭을 유도할만한 제목을 만들어줘.
        - 글 제목에 " 스토리 작가 " 언급 하지마
        - **쉼표(,)와 느낌표(!) 사용 절대 금지
...
async def main():
    ...
    # 원신에 대한 나의 핵심 비판 논리
    - 여기 써 있는 이유중 랜덤으로 주제를 뽑아 작성한다 ( 똑같은 주제를 연속해서 쓰지않고 매번 다른 주제를 사용한다)
    - 원신 지금이라도 유기하고 붕스에 올인하는게 호요버스 미래를 위한 길이다
    ...
    gallery_record_interval = 1000 # <--- 이 변수가 이제 활성화되어 900초마다 주제 크롤링을 제어합니다.
